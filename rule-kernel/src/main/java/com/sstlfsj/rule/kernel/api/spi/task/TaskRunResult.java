package com.sstlfsj.rule.kernel.api.spi.task;

/**
 * 一次任务执行的结果(executor 返回,由调度框架写 scheduled_task_execution)。
 *
 * @param status         终态
 * @param processedCount 处理总数(TRIGGER:主体 / INGESTION:标签行)
 * @param successCount   成功数
 * @param errorCount     失败数
 * @param errorSummary   错误摘要(可空)
 * @param newCursor      推进后的运行游标(增量任务用;null 或等于原值表示未变,框架据此决定是否写回 run_cursor)
 */
public record TaskRunResult(TaskExecutionStatus status, int processedCount, int successCount,
                            int errorCount, String errorSummary, String newCursor) {}
