...
class MetricVersionResolveTest {
    ...
    void resolvesBoundVersion_singleVersionSteadyState() {
        ...
        asm.assemble(event(Map.of("account.age", 5)), List.of(rule), new EvalEnv(NOW, java.util.Map.of()));
    ...
    void resolvesBoundVersion_contextHasMetric() {
        ...
        EvalContext ctx = asm.assemble(event(Map.of("account.age", 5)), List.of(rule), new EvalEnv(NOW, java.util.Map.of()));
    ...
    void multiVersionTransition_picksHighest() {
        ...
        asm.assemble(event(Map.of("account.age", 5)), List.of(ruleA, ruleB), new EvalEnv(NOW, java.util.Map.of()));
