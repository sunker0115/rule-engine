...
class LteEvaluatorTest {

    private final LteEvaluator evaluator = new LteEvaluator();
    ...
    private ConditionNode node(String metric, Object threshold) {
        return new ConditionNode("LTE", metric, "", Map.of("threshold", threshold), 0.0);
    }

    @Test
    void actualLess_returnsTrue() {
    ...
    void annotation_describesOperator() {
        var ann = LteEvaluator.class.getAnnotation(
                com.sstlfsj.rule.kernel.api.annotation.ConditionType.class);
        assertThat(ann).isNotNull();
        assertThat(ann.value()).isEqualTo(ConditionTypes.LTE);
