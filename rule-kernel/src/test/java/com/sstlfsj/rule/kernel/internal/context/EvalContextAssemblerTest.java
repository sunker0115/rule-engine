...
class EvalContextAssemblerTest {
    ...
    void noSubjectLoader_buildsMinimalSubject() {
        ...
        EvalContext ctx = assembler.assemble(event(Map.of("score", 100)), List.of(), NOW, java.util.Map.of());
    ...
    void providedMetrics_populatedInContext() {
        EvalContextAssembler assembler = new EvalContextAssembler(List.of(), List.of());
        EvalContext ctx = assembler.assemble(event(Map.of("score", 42, "tag", "vip")), List.of(), NOW, java.util.Map.of());
    ...
    void withSubjectLoader_usesLoader() {
        ...
        EvalContext ctx = assembler.assemble(event(Map.of()), List.of(), NOW, java.util.Map.of());
    ...
    void metricDependency_extractsCodeFromObject() {
        ...
        EvalContextAssembler assembler = new EvalContextAssembler(List.of(), List.of());
        EvalContext ctx = assembler.assemble(event(Map.of("balance", 500)), List.of(snap), NOW, java.util.Map.of());
    ...
    void allowProvided_false_ignoresProvidedValue_andFallsToError() {
        ...
        EvalContext ctx = assembler.assemble(event(Map.of("kyc", 99)), List.of(snap), NOW, java.util.Map.of());
