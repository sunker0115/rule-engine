...
CREATE TABLE IF NOT EXISTS job_definition (
  ...
  subject_query    JSON         NOT NULL COMMENT '主体查询配置（D48），如 {"type":"BEAN_METHOD","ref":"<bean>#<method>"}',
  event_type       VARCHAR(64)  NOT NULL COMMENT '合成 RuleEvent 使用的 eventType',
  payload_template JSON         COMMENT 'D49 遗留列，已不再使用（payload 由 @RuleJob 方法返回的 JobTarget.payload 携带）',
