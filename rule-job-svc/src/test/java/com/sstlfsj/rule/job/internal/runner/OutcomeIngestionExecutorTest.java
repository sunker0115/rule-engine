...
class OutcomeIngestionExecutorTest {
    ...
    @Test
    void cursorAdvanced_writesBackRunCursorColumn() {
        ...
        // 游标存于 task.run_cursor 列(非 config);executor 先 selectById 读游标解析为 watermark
        ScheduledTask task = new ScheduledTask();
        task.setConfig(config);
        task.setRunCursor(oldWatermark.toString());
        ...
        // ingest 用 task.run_cursor 解析出的 watermark 调用,不是 config
        ...
        // run_cursor 列前进到新 watermark;config 不动(仍是原 OutcomeIngestionConfig)
        assertThat(captor.getValue().getRunCursor()).isEqualTo(newWatermark.toString());
    ...
    void cursorUnchanged_noWriteBack() {
        ...
        ScheduledTask task = new ScheduledTask();
        task.setConfig(config);
        task.setRunCursor(watermark.toString());
        when(taskMapper.selectById(TASK_ID)).thenReturn(task);
        // 空批次:newWatermark == 旧 run_cursor,不应写回
