 ...
 * {@code @TriggerTask} 分页方法，直到方法返回空批为止；每批处理完再拉下一批，内存只占一批。
 *
 * <p>分页方法签名形如 {@code List<SubjectTarget> m(SubjectPage page)}，方法体用 {@code page.offset()} /
 * {@code page.pageSize()} 作 SQL {@code LIMIT ... OFFSET ...}。
 *
 * @param pageNumber 页码，从 0 开始递增
 * @param pageSize   框架建议的每页条数（方法可据此 LIMIT）
 */
public record SubjectPage(int pageNumber, int pageSize) {
