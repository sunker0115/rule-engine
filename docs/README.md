...
- 动作以"发券 / 发消息 / 调 webhook / 写入业务表"为主，声明式动作可以覆盖 70%。
...
- **动作编排（工作流）** —— v1 动作是平铺 forEach 顺序执行；v2 接 Camunda / Flowable 时 `ActionHandler` 协议要能被工作流引擎当作"任务节点"复用，而非彻底重做。

**明确不预留**（场景外）：

- 不做实时 OLAP 类指标（如"全网过去 1 分钟平均价格"）—— 这不是规则引擎职责，由外部指标平台供给；
- 不做动作的 SAGA / 强事务编排 —— 用补偿流水线处理"撤销"，不做分布式事务。

---

## 二、核心设计决策（已落定，逐条权衡见 [`00-decisions.md`](./00-decisions.md)）

下表与 `00-decisions.md` 的 D1-D21 一一对应；"选择"列是最终落定，"取舍"列概括为什么这么选。
...
| D4 | **动作协议** | **声明式优先 + SPI 兜底**：webhook / MQ.send / SQL.update / log 配置即开；复杂逻辑 Java `@ActionType` 注册 | 80% 场景零代码上线；编排（顺序/并行/分支/等待）留到 v2 接工作流引擎 |
...
| D7 | **Dry-run 一等公民** | 走完整评估链路，仅 ActionHandler 短路；前端有专门试算面板 | v1 先做"评估层试算 + 节点级 trace"，动作层 dryRun 在 v1.5 补齐到全 ActionHandler |
...
| D15 | **评估失败语义** | **单节点失败 → 整树继续按短路求值** + `EvalResult.errorCode` 槽位 + 规则间隔离 + `HIT/MISS/ERROR` 三态对账 | PUSH 默认安静失败不派发 Action；PULL 返回 `{satisfied, errorCode}`，调用方按业务策略（fail-secure / fail-open）决策 |
...
│  扫 AST 收集涉及的 metricCode → MetricRegistry 并发取数 → Context │
└─────────────────────────────────┬────────────────────────────────┘
                                  ▼
┌──────────────────────────────────────────────────────────────────┐
...
│   └─ 写 action_execution 表                                       │
└──────────────────────────────────────────────────────────────────┘

旁路:
  • Compensation Pipeline  (对称的反向流水线, 处理"撤销/退回")
  • System Audit           (系统行为: evaluation_session + node_trace + action_execution)
  • Operation Audit        (人的行为: audit_log, D14, 与系统行为严格分离)
  • Idempotency Guard      (Redis trySet + DB uk 双兜底)
  • Versioning & Rollout   (规则版本快照 + 灰度命中)
  • Metric Aggregator      (规则命中率 / 动作成功率 / 延迟分位 → Prometheus)
```
...
| 02-runtime | "一个事件进来后，引擎内部发生了什么？" | ⏳ 占位草稿 |
| 03-rule-expression | "我能写多复杂的规则？怎么表达嵌套逻辑？" | ⏳ 占位草稿 |
| 04-extension | "我要加一个新条件 / 动作 / 指标，从哪下手？" | ⏳ 占位草稿 |
| 05-storage | "数据怎么存？我能直接查 SQL 看到什么？" | ⏳ 占位草稿 |
| 06-frontend | "运营怎么自助配规则？前端怎么和后端对齐？" | ⏳ 占位草稿 |
| 07-operability | "上线后出问题怎么排？怎么灰度？怎么试算？v1 可用性边界？" | ⏳ 占位草稿 |
| 08-evolution | "现在做到哪一步？为什么没选 XXX 方案？" | ⏳ 部分展开（§2.1 / §2.12 / §2.13 / §2.14 / §2.15 演进锚点已就位；§三 决策时间线 / §四 已否决方案待展开） |
| 09-skeleton | "新代码放哪个模块/包？SPI 接口在哪？业务方依赖哪个 jar？测试怎么组织？" | ⏳ 占位草稿（章节就位，内部具体决策待定） |
| 10-api-contract | "调用方要传什么 / 收到什么？有哪些 errorCode？SDK 怎么用？" | ⏳ 占位草稿 |
...
7. **试算与真实执行同源** —— Dry-run 走完整评估链路，仅在 ActionHandler 层短路；保证试算结果和真实执行一致。
8. **正反对称** —— 执行链路有什么，补偿链路就有什么（Matcher / Gate / Handler 都有 reverse 版本）。
9. **多租户从 schema 起** —— `tenant_id` 是所有表的第一个字段，索引前缀必须包含它。
...
| 2026-05-25 | **D15 落定：评估失败语义**。单 ConditionNode 失败 → satisfied=false 但整树继续按 AND/OR/NOT 短路求值；`EvalResult` 加 `errorCode` / `errorMessage` / `failedNodeIds` / `partial` 槽位；规则间隔离（单条失败不影响 (scene+eventType) 下其他规则）；对账分 `HIT / MISS / ERROR` 三态，ERROR 不计入命中率分母。PUSH 模式安静失败不派发 Action；PULL 模式返回 `{satisfied, errorCode}` 调用方按 fail-secure / fail-open 决策。§四 抽象表 `EvalResult` 行同步。 |
...
| 2026-05-25 | **D17 落定：配置热加载**。DB 短轮询（默认 15s，可配 `engine.rule.poll-interval`）+ 内存倒排索引 `(scene, eventType) → List<RuleVersionSnapshot>`；evaluation_session 开始时拍快照，整 session 用同版本（D6 派生）；多实例最终一致，毫秒~15s 窗口；灰度桶基于 `(subjectId, ruleVersionId)` hash 不依赖实例。`RuleVersionWatcher` 接口预留，v2 可换 MQ / Nacos。§四 抽象表追加 `RuleVersionWatcher`。 |
| 2026-05-25 | **D18 落定：Action 失败补偿语义**。默认 continue-on-error，单 Action 失败不影响同 Rule 后续 Action 也不影响 `EvalResult.satisfied`；`retryable=true` 入重试队列，`retryable=false` 直接落 FAILED；Action 级可声明 `failFast=true`，失败后同 Rule 内 `sortOrder` 大于本 Action 的后续 Action 标 SKIPPED；补偿不自动触发，由 D4 补偿流水线外部调度。§3.7 Action 关键边界 + §四 抽象表 ActionResult 行同步。 |
| 2026-05-25 | **D19 落定：规则发布事务性**。单条规则原子发布（状态机 `DRAFT → PUBLISHING → PUBLISHED / PUBLISH_FAILED`），单 DB 事务内完成状态机迁移 + `rule_version` 新版本快照行 + audit_log；批量发布由前端逐条提交，v1 不提供批量原子 API；回滚 = 用旧版本快照建新草稿走标准发布产新版本号，不可变快照永不覆盖。§3.4 Rule 字段表 status 枚举扩展 + §四 抽象表追加 `RuleVersion`。 |
...
| 2026-05-26 | **v1 缺口补充（基于市场对比 + 评审反馈）**：(1) D20 §2 明示 v1 全局单一消费者线程池，Scene 级隔离留 §2.13；(2) D20 §1 + §3.9 metric 预拉值评估期内冻结，跨 TTL 边界仍读初始快照；(3) §3.8 补 EvalContext 7 个标准字段类型 + 语义表（D20 §3 闭合校验根集）；(4) §3.7 `ActionResult.errorCode` 追加 `TIMEOUT`；(5) D19 v1 落地范围追加 PUBLISHING 残留兜底清扫（新 `audit_log.after_snapshot.errorCode=ZOMBIE_PUBLISHING`）；(6) §3.11 audit_log 关键边界登记 `after_snapshot.errorCode` 已知值清单（`UNRESOLVED_VARIABLE` / `ZOMBIE_PUBLISHING` / `HANDLER_EXCEPTION`）；(7) §3.10 Job 关键边界明示灰度桶在引擎 Pre-Gate 计算（D6 + D11 派生）。 |
...
| 2026-05-26 | **v1 取舍显化**：(1) D19 决策顶部追加发布状态机 ASCII 摘要（DRAFT→PUBLISHING→PUBLISHED/PUBLISH_FAILED + PUBLISHED↔DISABLED），统一散落各处的状态文字描述；(2) D20 §2 追加重试上限归宿（达上限 → FAILED 终态 + 走 D4 补偿，v1 不引入 DLQ）+ 进程重启数据丢失依赖上游可重推说明；(3) D17 多实例一致性段显式标注"单 session 强一致 + 跨 session 最终一致是 by design"，避免读者误以为是缺陷。 |
