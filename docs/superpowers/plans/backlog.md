...
| # | 功能 | 来源 | 备注 |
|---|------|------|------|
| B15 | **规则模板市场** | 08-evo §2.10 | **参数化模板目录 + 租户按参实例化**，≠ B7 导出/导入（B7 搬"具体规则"原样复制，仅作 B15 的搬运层）。前置 D74 模板系统 V2 **已落地**（platform 层规则模板、跨租户可见性、实例化流水线、SlotRefResolver）。模板市场是对平台模板的发现/评分/分类层——v3 范畴 |
