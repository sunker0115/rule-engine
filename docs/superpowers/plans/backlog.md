...
| 序 | # | 功能 | 来源 | 预计改动范围 | 备注 |
|---|---|------|------|------------|------|
| 1 | B19 | **类型化比较策略工厂**（`ComparisonStrategy` 按 dataType 路由） | trae R3 | `rule-kernel`：`ComparisonStrategyFactory.forType(dataType)` + 5 种策略类（Numeric/String/Boolean/List/Date）；替换 `ConditionEvaluator` 内部的 instanceof if-else | 最小，给 B1/B6 的多类型处理铺路；随 `ConditionEvaluator` 多类型批次落地，不单独开 task；trae `rule/condition/strategy/` 有完整参考实现 |
...
| 4 | B6 | **Metric 版本化** | 08-evo §2.2 | `metric_definition` 加 `version` 列；`rule_version.metric_dependencies` JSON 升级；发布期 + 评估期解析逻辑；运营 UI 影响面展示 | 正确性兜底：Metric 语义变更时防止存量规则静默错误的根本解法；改动面横跨发布期/评估期两端 |
| 5 | B7 | **规则导出 / 导入** | 08-evo §2.9 | 独立工具链；导出格式 JSON Bundle；导入幂等写入 + 权限校验；无核心引擎变动 | 风险最低（不动引擎），可穿插做；跨环境迁移、Incident 复现的基础工具；**解锁 B15** |
| 6 | B20 | **时间框架**（now 时钟 + time.window/time.occurred_at + DATE/DATETIME + $now/$today + 时区） | 03-rule-expression §7/§3.4（设计已全，几乎未实装） | `EvalContext` 加注入式 `now`（统一时钟 + dry-run 快照重放）；`time.window`/`time.occurred_at` 两个内置 conditionType；dataType 枚举 +`DATE`/`DATETIME`（ALTER + `01-concepts`/`05-storage` 文档）；DATE_* 经时间策略并入 B19 矩阵；`$now/$today` 解析 + 时区解析序（字面量 offset>条件 tz>Scene 默认>UTC） | §7 三模型几乎全未实装、连 `EvalContext.now` 都没有；近N天聚合留 SQL（见 B21）；**依赖 B19 框架**；设计稿 `specs/2026-06-05-time-framework-design.md` |
| 7 | B21 | **FETCHED 取数层 + SQL/HTTP 指标范式** | `EvalContextAssembler` 空壳 / §7.3 | `EvalContextAssembler` 真正调 `MetricSourceHandler`（并发装配 + cacheTtl 缓存 + 失败降级）；SQL 范式：命名参数（`:subjectId/:tenantId/:now/:payload.*`）禁拼接、`:now` 绑引擎时钟（非 DB `NOW()`，保 dry-run 重放）、滚动窗口长度写在 SQL 文本；结果首行首列按 dataType 强转 | 今天所有指标只能 PROVIDED（assembler 不调 handler）；"近N天"真正可用的前提；比时间框架更底层；B20 设计时发现 |
