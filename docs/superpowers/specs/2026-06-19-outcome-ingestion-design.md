...
- 定时(`scheduled_task` TRIGGER 之外的 `OUTCOME_INGESTION` 型)从业务真值源按 watermark(存 `scheduled_task.cursor` 列,非 config)**增量批量**拉标签行,经 `OutcomeService.recordOutcomes` 幂等 upsert。
...
| `OutcomeIngestionConfig` | **job-svc api** | `implements TaskConfig`(sealed permit,同包);静态定义只持 `OutcomeSourceConfig source`(无 watermark——运行态游标见 `scheduled_task.cursor` 列) |
| `OutcomeIngestionExecutor` | **job-svc internal** | `TaskExecutor<OutcomeIngestionConfig>`;调 eval-svc `OutcomeIngestionService` + 读/写 `scheduled_task.cursor` 列 |

## 3. SPI 微调:`TaskExecutor.execute(TaskRunContext, config)`

当前 `execute(long taskRunId, long tenantId, C config)` 不给 taskId,而 OUTCOME_INGESTION executor 要把新 watermark **写回 `scheduled_task.cursor` 列**(需 taskId 定位行)。改为传上下文:
...
运行态 watermark **不在 config**(config 是不可变静态定义),存于独立 `scheduled_task.cursor` 列(opaque VARCHAR,OUTCOME_INGESTION 存 ISO-8601 watermark)——对齐 Kafka Connect offset / Airbyte state 的 state-not-config 模式。

**`OutcomeIngestionExecutor implements TaskExecutor<OutcomeIngestionConfig>`**(job-svc internal):
- 注入 `OutcomeIngestionService`(eval-svc)+ `ScheduledTaskMapper`(读/写 cursor 列)。
- `execute(ctx, config)`:
  1. `selectById(ctx.taskId())` 取 task;null 则返回 FAILED。从 `task.getCursor()` 解析 watermark(null=首次全量)。
  2. `IngestResult r = ingestionService.ingest(ctx.tenantId(), config.source(), watermark)`。
  3. **游标前进则写回**:`r.newWatermark() != watermark` 时,`task.setCursor(r.newWatermark().toString())` → `updateById`(只更新 cursor 列,config 不动)。
...
- **集成测试**:扩 `ScheduledTaskAnnotationIntegrationTest` 或新增——真实 MySQL:建一张业务标签表 + 一条 OUTCOME_INGESTION `scheduled_task`,`runOnce` → 验 `decision_outcome` 真 upsert + watermark 写回 `scheduled_task.cursor` 列。
