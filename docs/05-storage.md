...
CREATE TABLE rule_version (
  ...
  body                  JSON         NOT NULL COMMENT '判定主体多态载体 RuleBody（D76，V1_40）：AstBody{conditionAst}(AST 系四形态) / ScriptBody{script}(EXPRESSION_SCRIPT) / FlowBody{flowGraph,referencedSnapshots}(DECISION_FLOW) 三选一，type 判别，与 kind 一致；三承载收敛（原 condition_ast/script_source/flow_graph/referenced_snapshots 四列，V1_40 合并）',
...
CREATE TABLE scheduled_task (
  ...
  name        VARCHAR(128) NOT NULL,
  task_type   VARCHAR(32)  NOT NULL COMMENT '开放类型名（去中心化，与 TaskExecutor.type() 对应）：TRIGGER / OUTCOME_INGESTION / ...',
  cron        VARCHAR(128) NOT NULL COMMENT 'Spring 6 段 cron；seed 初值，XXL admin 运行时权威',
  config      JSON         NOT NULL COMMENT '原始 JSON 配置（框架核不透明；各 handler 派发时按 executor.configType() 反序列化成自带 typed record，静态定义）；TRIGGER=TriggerConfig{sceneCode, eventType, subjectQuery}，OUTCOME_INGESTION=OutcomeIngestionConfig{source(SQL: datasource, sql)}',
