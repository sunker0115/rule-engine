...
# 2) 注册 metric(tenantId/metricCode 走 query,body 不含这两者)
curl -s $H -X POST "$BASE/admin/v1/metrics?tenantId=9001&metricCode=amount" \
  -d '{"name":"交易金额","sourceType":"ATTRIBUTE","dataType":"LONG","params":{},"cacheTtlSeconds":0,"allowProvided":true}'
