...
| 待定 | 变量补全（Level 1） | Monaco CompletionItemProvider 喂 `inputManifest` + metric 列表 | 纯前端零后端，建议做 |
| 待定 | CEL 实时类型诊断（Level 2） | keystroke 级红波浪，复用 CEL `typeCheck` + 编译缓存（成本≈0） | `POST /expressions/validate` + 前端 debounced |
