...
| `config` | typed `TaskConfig`（DDL 单 JSON 列，sealed 类型族，多态 `kind` 判别）。`TRIGGER` 为 `TriggerConfig{sceneCode, eventType, subjectQuery}`（`sceneCode` 关联 scene.code、PULL Scene 拒绝绑定；`subjectQuery.type=BEAN_METHOD` 指向 `@TriggerTask` 注解方法）；`OUTCOME_INGESTION`（B32 标签回灌）为 `OutcomeIngestionConfig{source, watermark}`——`source` 为多态 `OutcomeSourceConfig`（首个实现 `SqlOutcomeSourceConfig{datasource, sql}`），`watermark` 为运行态游标（每轮取本批 max `labeledAt` 写回，推进增量） |
