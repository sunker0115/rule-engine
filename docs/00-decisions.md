...
**后续实装补记**：`SCORECARD` evaluator 已实装（D12 决策时）；`DECISION_TREE` / `DECISION_TABLE` evaluator 已实装（D42）；`ALL_HITS` / `FIRST_HIT` executionStrategy 已实装（D41）；`EXPRESSION_SCRIPT` evaluator 已实装（D66，六表达式引擎）。
...
**前向兼容（嵌入式 SDK 取数 B2，见 `specs/archive/2026-06-06-sdk-fetch-design.md`）**：① metric 定义是独立可下发配置，不冻进 `rule_version` 快照；② `MetricDefinitionResolver` 数据源无关（服务端读库 / 嵌入式读下发缓存共用）；③ `EvalContextAssembler` 富构造为服务端与 SDK 统一取数入口（旧 2 参构造保留为 providedMetrics-only 退化路径）；④ `MetricDescriptor` 为定义下发的序列化契约。
...
**背景**：B21（D45）把取数管线建在数据源无关的 `MetricDefinitionResolver` / `MetricSourceHandler` / `MetricCache` SPI 上，但嵌入式 `RuleEngineClient` 仍用旧 2 参 `EvalContextAssembler` → 仅 providedMetrics 生效、不取数。设计见 `specs/archive/2026-06-06-sdk-fetch-design.md`（设计冻结 2026-06-06，本条落地）。
...
**背景**：依赖范围 A（payload 直接引用，`valueRef` 已把 payload 事实 / metric 指标分开）之上，给"对场景发事件"的调用方一个精确可校验的输入契约——查一次该场景要传哪些事件事实（名 + 类型），照着传 `payload`，剩下引擎自己搞定。设计见 `specs/archive/2026-06-10-scene-input-manifest-design.md`（brainstorming 冻结 2026-06-10，本条落地）。
...
**背景**：D6（版本与灰度）+ D19（规则发布事务性）确立了"快照不可变 + 回滚 = 用旧版本快照建新草稿"，但现状实现把生命周期压成了一次性链路（createDraft 存 raw 未解析草稿 → publish 时才解析 + INSERT 新 ACTIVE 行 → 之后只能 disable），且 dry-run 复用评估主链路会落 `evaluation_session` / 派发 action（副作用 bug）。本决策补全生命周期、把"草稿即完整冻结快照"立为 premise，并重设计 dry-run。设计见 `specs/archive/2026-06-10-rule-draft-version-lifecycle-design.md`（设计冻结 2026-06-10，本条落地）。
...
| D69 | Schema 线收口:typed 契约 + 分层校验(承接 D68) | A | 把"建场景/写规则/发事件"三处松散输入收成 typed 契约 + 建模期/发布期/运行期分层校验,对标 DMN(itemDefinition+typeRef)/Calcite(SqlOperatorTable+OperandTypeChecker)/JSON Schema。**算子半**:新增 config-svc `ConditionTypeCatalog`(每内置算子的必填 param 键+允许 dataType+requiresMetric+显示名,单一源)→ `ConditionParamValidator` 发布期校必填键(堵 D68 留的 API raw-JSON 错键静默死规则)、`AstDataTypeResolver` 收敛读 catalog、`MetadataService.getSceneMetadata` 填 conditionTypes+paramsSchema(补 v1 空占位)。**payload 半(模型 2:约束随规则发布冻结)**:`PayloadFieldType` enum + scene 创建/更新期 type 校验(裸 String→封闭集,堵拼错静默落 UNKNOWN)+ `@JsonInclude(NON_NULL)` 止血;`PayloadDependency` 扩 enum/min/max/pattern(+ Lombok @Builder + 3 参兼容构造器),发布期从 scene.payloadSchema 冻全量约束进 `payload_dependencies`;`PayloadInputValidator` 运行期强制 enum/min/max/pattern(pattern 用 RE2J+编译缓存,基准证可忽略)。**审计收口**:删 `scene_payload_schema_history` 表 + `scene.payload_schema_version` 列(`V1_30`)+ 全仓退役 version 概念;scene create/update/disable 改走 `OperationAuditedEvent` 前后快照(新 `SceneSnapshot` implements AuditSnapshot)落 `audit_log`,比旧专用表记得更全。**编码约定固化**:多参值类型走 Lombok `@Builder`(写进全局规范)。验证:27 模块全绿 + 真实服务 DB 端到端(坏 type→400、合法 schema→200、metadata 17 算子、audit 前后快照含 payloadSchema、历史表/列真删)。**后续全部落地**:① `scene.default_params` 接入评估(D69后续①,`EvalEnv` 统一 now+sceneDefaultParams + 可传 asOf 求值时钟 + `updateScene` 发 `SceneChangedEvent` live 传播;design:`specs/archive/2026-06-13-time-evaluation-correctness-design.md`)。② `OperatorCatalog` 合成单一源:新增 kernel `api/operator/OperatorSpec`(@Builder record)+ `ConditionTypeCatalog` 移入 kernel `internal/condition` 从 evaluators 动态构建(删 config-svc 硬编码 map)。③ `ConditionEvaluator.spec()` default 方法:19 个内置 evaluator 各自 override(算子契约住在实现里,单一真相源);自定义 SPI opt-in;spec 驱动发布校验 + 元数据暴露(D69②+③)。DB 端到端全通。设计见 `specs/archive/2026-06-13-schema-line-consolidation-design.md`。 |
| D70 | 忠实重放:历史评估的时间旅行式重现 | A | 给历史 `evaluation_session` 加**忠实重放**:锁当时规则版本 + 灌当时 payload/metric/evalNow + 跳过取数,重跑出与当时一致的 `EvalResult`+trace,**只读零副作用**(不落新 session、不触发下游)。**落库侧**:`evaluation_session` 加 `payload`、`candidate_rule_version_ids` 两列(V1_31,可空兼容存量;`AuditRecordedEvent` 补候选版本 id),metric/evalNow 复用现有 `context_snapshot`,规则版本复用不可变版本库(D56 SUPERSEDED 保留)。**捕获开关默认开**:三件套(payload+候选id+context_snapshot)捕获并入 `engine.rule.audit.context-snapshot.enabled`,默认由关改**开**(session 默认可重放,可配置关闭)。**回放侧**:`POST /admin/v1/evaluation-sessions/{sessionId}/replay` + `ReplayService` + 新引擎入口 `EvalEngine.evaluateReplay`(抽 `evaluate0` 共享核心;degraded `EvalContextAssembler` 把冻结 metric 当 providedMetrics 回灌、绕过取数——与 dry-run 重新取数的本质区别)。**忠实度边界**:subject 暂重载当前、metric 仅存 rawValue(丢 dataType/isError)、本特性上线前存量 session 与 best-effort 丢失 snapshot 的不可 replay(`REPLAY_NOT_REPRODUCIBLE`,不退化成当前数据)。**非目标**:what-if(历史数据×当前规则)留后续(复用快照回灌、改锁版本为取当前版本)。设计见 `specs/archive/2026-06-12-faithful-replay-design.md`。 |
| D71 | Trace PII 读时脱敏(展示出口遮蔽) | A | `node_trace.actualValue` 与 dry-run/replay 返回 trace 含 PII(payload 原值 / 敏感 metric 值,D70 后 raw 默认落库)。做**读时脱敏**:raw 照常落库(**不碰落库路径/重放忠实度**)。**与上文「敏感数据加密/脱敏 v1 不做」分层互补,非冲突**——库内仍按原值落库不变,字段级加密/审计 hash chain 仍留合规阶段;本条只在 rule-api **展示出口**新增把敏感叶子 `actualValue` 抹成 `"***"`。**声明各在定义点**:payload 字段敏感性在 scene payloadSchema(`PayloadFieldSpec.sensitive`,随 JSON 存无 DDL),metric 在 metric 定义(`metric_definition.sensitive` 列,V1_33;`sensitive` 为 MySQL 保留字,DDL 与 `@TableField` 加反引号),metric 租户级共享(D54)。**live 当前策略**(非冻结进版本快照——脱敏只影响展示不影响判定,事后标敏感即刻回溯遮蔽历史),故 **node_trace 表 / kernel `NodeTrace` / `TraceWriter` 全不动**。`SceneService.getSensitiveRefs(tenant,scene)→SensitiveRefs(payloadFields,metricCodes)` 取 live 敏感集;rule-api `TraceMasker`(maskFlat/maskTree/maskKernel 三形状纯函数,返回新结构)按 `valueSource`(PAYLOAD→payload 字段集,FETCHED/PROVIDED→metric 码集)+`metricCode` 匹配,接入 trace 扁平/树查询(`AuditController` 经 `AuditService.getSessionSceneCode` 解析场景)+ dry-run(`EvalController`)+ replay(`ReplayController`)四出口。**fail-closed**:敏感集查询失败 / 会话场景缺失 → `refs==null` 全抹,宁过度遮蔽不漏 PII。**非目标**:角色级看原值豁免(引擎无 RBAC,D14 鉴权交网关)、部分遮蔽(138****1234)/ 哈希。对标 DB 动态数据脱敏 / Apache Ranger 列脱敏(声明在字段定义、读时按策略遮蔽)。设计见 `specs/archive/2026-06-13-trace-pii-masking-design.md`。 |
| D72 | EXTERNAL_HTTP 取数标准化:声明式连接器(可复用命名资源) | A | 对标 OpenFeature(provider SPI + spec + conformance)/ OTel(exporter SPI + OTLP 信封 + semantic conventions)。把"多团队各 fork 一套 `EXTERNAL_HTTP`、靠口头对齐、联调贵"归一为**声明式连接器**。**① 连接器=可复用命名资源,不塞 `metric.params`**:新增 `connector_definition` 表(V1_34:`connector_code` 租户内唯一 + `name` + `descriptor` JSON 列 + `status` VARCHAR ACTIVE/DISABLED,模板=`RuleVersion` 的 `@TableName(autoResultMap=true)`+`Jackson3TypeHandler`),实体 `ConnectorDefinition`;metric 经 `params={connector:"<code>", vars:{...}}` 引用,信封映射定义一次多 metric 复用、可单独联调。塞进 metric 会让映射复制 N 份。**② v1 可变 + 热加载,不做 per-version 冻结**:连接器变更走现有事件失效机制——`ConnectorChangedEvent` → `ConnectorDefinitionResolver.invalidate`(镜像 `MetricDefinitionResolver`,DB 读 + Caffeine 60s 负缓存),不重启、不冻结(像 endpoint/datasource);以简为先,冻结等真咬到再加。**③ 共用脊调用无关(跨源)**:`VariableRenderer`(统一命名空间 `payload/params/vars/subject/subjectId/tenantId/now`,HTTP 占位符 `{ns.key}` 与 SQL 命名参数 `:ns.key` 对称)、`ResiliencePolicy`(+`ResiliencePolicyExecutor`)、`MetricFetchError`(kernel closed enum:`NOT_FOUND/TIMEOUT/UNAUTHORIZED/UPSTREAM_ERROR/PARSE_ERROR/MAPPING_ERROR/TYPE_MISMATCH`)、`MetricFetchTester` 均做成**外部调用无关**,HTTP 与 SQL 两源共担,为未来 MODEL_SCORE/LLMCondition/SubjectLoader 增量接入不返工(边界纪律 A)。SQL 侧只继承共用脊(错误细码 + statement 超时 + `:subject.*` + 自助测试),**不加对称的"SQL 连接器中间层"**(SQL 无请求形态/信封/鉴权变异,加层是为对称而对称)。**④ 声明式描述符(全 typed record,禁 Map/String)**:`ConnectorDescriptor{endpointRef, request:HttpRequestTemplate, response:ResponseMapping, auth:AuthScheme, resilience:ResiliencePolicy, errorMapping:List<ErrorRule>}`——`endpointRef` 指向 infra 已注册命名端点(`HttpEndpointRegistry`,密钥只在这层,灭 SSRF);`request` 含 `method/pathTemplate/query/headers/bodyTemplate`(均走统一渲染器);`response` 为 `{successWhen:Predicate(path,op,value), valuePath}`(点号 jsonPath + 声明式比较,不引表达式引擎);`auth` 三 kind 判别 `STATIC_HEADER/BEARER/OAUTH2_CLIENT_CREDENTIALS`(密钥以 `*Ref` 引用 infra,不内联;OAuth2 client-credentials 由 `OAuth2TokenManager` 换 token + 缓存);`resilience` 含超时/重试/熔断(熔断 v1 未实装,留位);`errorMapping` 按状态区间/信封码顺序匹配映射细码。实现 `DeclarativeHttpConnectorHandler`(重构自 `ExternalHttpMetricSourceHandler`)。**⑤ errorMapping 细码不改 D15 降级**:所有失败仍归一为 error `MetricValue`、引用节点不命中、整树短路继续、引擎不重试;`MetricFetchError` 细码只进 `MetricValue.errorCode`(开放 String 穿透)+ 可观测标签 `error_code` 维度,`METRIC_FETCH_FAIL` 保留为评估语义层伞码。**⑥ 一致性 + 自助测试**:带编号连接器契约落 04-extension §4.5(C1–C5,接入方 + 套件单一真相源,仿 OpenFeature spec);`rule-connector-conformance`(嵌入式 mock 上游 + 黄金用例);跨源自助测试端点 `POST /admin/v1/metrics/{code}:test` 与 `POST /admin/v1/connectors/{code}:test`(`MetricFetchTester` 实打实发一次,返回分阶段 trace),前端编辑器内联此面板。**⑦ AI/ML 模型打分 + 一等 CEP 序列模式只进演进位,本轮不实现**:模型打分本质=声明式 HTTP 连接器(新 sourceType 复用脊)、CEP 产出当 metric(`STREAM`)近期可落,但一等事件序列模式是新 evaluation kind、绝不为它掰弯取数脊——仅记录边界(YAGNI,本轮范围红线)。设计见 `specs/archive/2026-06-15-metric-fetch-standardization-design.md`;契约见 `04-extension.md` §4.5。 |
| D73 | job_definition 评估触发聚合重写为通用 `ScheduledTask` + `TaskExecutor` SPI 框架 | A | 把 `job_definition`（评估耦合聚合，`scene_code`/`event_type`/`subject_query` 全 NOT NULL + `JobRunner` 硬绑"合成 RuleEvent→评估"）重写为通用调度任务框架：单表 `scheduled_task`（`task_type` VARCHAR 判别 + typed sealed `TaskConfig` JSON 多态 `kind`，TRIGGER 的 `scene_code`/`event_type`/`subject_query` 下沉进 `TriggerConfig`）+ `scheduled_task_execution`（`processed_count` 通用计数：TRIGGER=主体数 / OUTCOME_INGESTION=标签行数）。`TaskExecutor<C extends TaskConfig>` SPI 按 `TaskType`（封闭集，首期 `TRIGGER` / `OUTCOME_INGESTION`）路由，`TRIGGER` 为首个 executor（`JobRunner`→`TriggerExecutor`），加新调度工作=加一个 enum 值+一个 executor+一个 config record，零主干改动。重命名：`@RuleJob`→`@TriggerTask`、`JobDefinition`→`ScheduledTask`、`JobExecution`→`ScheduledTaskExecution`、`JobService`→`ScheduledTaskService`、`JobController`→`ScheduledTaskController`、`JobTarget`→`SubjectTarget`、`JobPage`→`SubjectPage`、`RuleJobScanner`→`ScheduledTaskScanner`、`JobScheduleManager`→`ScheduledTaskScheduleManager`、`JobStartupRegistrar`→`ScheduledTaskStartupRegistrar`；API `/admin/v1/jobs*`→`/admin/v1/scheduled-tasks*`（list/get/enable/disable/trigger/executions，对外术语统一"调度任务/scheduled-task"）。保留不动：`Scheduler` SPI、XXL 适配（`XxlJobSchedulerAdapter`/`HttpXxlJobAdminClient`）、`SubjectQuery`/`BeanMethodQuery`/`SubjectQueryRunner`、`EventIdHasher`（TRIGGER 取主体+合成事件机制收编进 `TriggerExecutor`）。迁移 V1_37（建 `scheduled_task`/`scheduled_task_execution`，drop `job_definition`/`job_execution`，greenfield 无生产数据需保留）。动因=为 OUTCOME_INGESTION（B32 回灌）等非评估调度工作提供干净抽象，守"调度 WHEN / 任务 WHAT / 评估"三层解耦，不再为非评估工作打补丁。**取代/作废 D47/D48/D49 的 job-as-评估触发聚合形态**（历史条目不改）。设计见 `specs/2026-06-19-distributed-ready-scheduling-and-propagation-design.md`（§3–§5）。 |
| D74 | 参数化规则模板（运营填空建实例 + 已有规则反向导出）——**实验性预实现（2026-07-24 重设计），默认关闭（`rule.template.enabled=false`），待真实业务场景触发** | A | 记录一次设计讨论的结论以备将来直接展开（对标 Drools guided rule template / decision table、GoRules JDM、Camunda DMN）。**需求**：平台预制"规则模板"（逻辑骨架 + 空槽）→ 运营选模板、填空、同时挑租户/场景/事件 → 生成一条具体规则；并支持反向"从已有规则导出成模板"。**核心架构判断（让整件事变便宜的前提）**：模板是"建规则"之上的一层 **authoring 便利，不是新的运行时概念**——实例化 = 模板 + 填的值 + 绑定 → 拼成标准建规则请求体 → **直接喂现有 `createDraft`（走 D56 `resolveAndValidate` 全量校验冻结）**，产出就是一条普通 DRAFT `RuleVersion`，发布/灰度/回滚全复用现有管线；**kernel / eval-svc / 评估链路零改动，模板只活在 config-svc 写侧**（同 D73「Job 只是 Trigger 适配器、不加抽象层」哲学）。**三个设计取舍（均取简单端、不关将来的门）**：① **空槽只开放填值**（阈值 / 名单 / 时间窗），逻辑结构（And/Or/Not 嵌套、加减条件）锁死——这才叫"填空"且天然 guardrail；"选哪个 metric/field"作为可选 slot 类型留后续，结构可编辑则属另一功能（规则编辑器），不在此列。② **模板声明需求、实例化时校验**：模板引用的 payload 字段 + metricCode **扫 `templateAst` 自动推导**（复用发布期扫描器），实例化时复用 `resolveAndValidate` 校验所选租户/场景是否满足，不满足直接复用现有 `UNRESOLVED_VARIABLE` / `METRIC_NOT_FOUND` 报错（"这模板装不进这个场景"）——模板保持租户/场景无关、可跨场景复用。③ **快照、不联动**：实例一次性生成，模板升级不回溯改老实例；实例只留 `templateId + templateVersion` 溯源，要更新就基于新模板重新实例化（对齐 D6 不可变版本）。**设计草图**：新增 config-svc `RuleTemplate` 实体（`code / name / description / kind / templateAst / slots / version / status`；`templateAst` 即正常 AST，某些 `params` 值位写占位符 `{"$slot":"<key>"}`；`slots` = `List<{key, label, dataType, required, default, constraint(min/max/enum)}>` 供前端渲染表单 + 校验填值）+ `RuleTemplateService`（复用 `RuleService.createDraft`）+ controller + MapStruct + 一张 `rule_template` 表；实例化 API `POST /admin/v1/rule-templates/{code}/instantiate {tenantId, sceneCode, triggerEventTypes, ruleCode, ruleName, slotValues}`；**反向导出 = 实例化的逆操作、共用同一 slot schema**（拷贝现成规则 AST → 剥掉租户/场景/事件绑定 → 让作者勾选哪些字面值挖成 `$slot`/哪些保持固定常量、起名配约束 → 需求扫 AST 自动推导）；`rule_version` 加 `template_id` / `template_version` 两列（nullable，手建规则为空）做溯源。**价值边界（为何暂缓）**：数据层与运行时**两者零区别（故意如此）**，价值全在 authoring——降低门槛 / 防错（运营碰不到 AST、不用懂 metricCode）、结构一致性 guardrail、同形状重复时省事；导出能力只降**供给侧**（模板库建立成本），**不创造复用需求**。**触发条件（满足再展开为 spec）**：产品确定要把"建规则"交给**非技术业务运营** + 出现**同一形状规则反复建**。二者不成立时（单人开发 / 技术作者看得懂 AST / 规则多为一次性），模板是为假想需求买单的无用间接层，违 §2 以简为先。本轮唯一落地动作 = 本条记录 + 守住"AST 逻辑保持绑定无关"这条既有边界（已基本守住）。**重设计（2026-07-24，取代上文"已启动 / v1 仅覆盖 AstBody 四 kind"措辞）**：定位退回**实验性预实现、默认关闭**（`rule.template.enabled=false`）——代码铺路已落地但不对外开放，待真实业务场景出现再开开关。**binder SPI 重设计**：改用 **JsonPointer 统一寻址**覆盖全 6 kind（AST 四形态 + EXPRESSION_SCRIPT + DECISION_FLOW），取代原"`templateAst` 内 `{"$slot":"<key>"}` 占位符"（只寻址 AST 槽位、Script/Flow 无处可挖的异类实现）；新增 **`params` 冻结常量命名空间**收编 Script/Flow——脚本 / 流图 body 无 AST 槽位，改由 params 注入冻结常量，消除该异类。实例化保持**快照式**（一次性生成规则版本，模板升级不回溯，对齐取舍③与 D6，非活链接联动）。 |
| D75 | 决策图编排层 `DECISION_FLOW`（第 6 种规则形态）——**设计定稿，待实现** | A | 对标 GoRules ZEN Engine 的 JDM（决策图 = Input→节点→Output 的 DAG，叶子是决策表、`Decision` 节点调子决策；图只做编排、叶子是原子）。填补现有扁平模型（Scene 下平铺规则 + 优先级合成）无法表达的一类需求：**一次评估内把多条决策串成有分支的流水线**（决策 A → 按结果分支 → 决策 B → 输出），跨规则复用 + 多步编排 + 按中间结果分流。**核心判断（加性、不重写）**：`DECISION_FLOW` 是并列的第 6 种 rule kind，**图只编排、叶子仍是现有 5 形态**（经 `RuleRef` 引用，作者用既有表单编辑器写），复用内核 / Metric SPI / 发布灰度血缘。**形态归属 = script 范式非 ast 范式**：body 走平级独立 typed 字段 `FlowGraph flowGraph`（`conditionAst=null`），全链路照抄 `scriptSource` 孪生位点。**四种节点最小集**：`RuleRef`（调一条已有规则）/ `Switch`（按表达式结果分支）/ `Transform`（表达式变换 context）/ `Output`（产出决策），`FlowNode` sealed + Jackson 多态仿 `AstNode`。**求值**：新增 `FlowExecutor implements RuleVersionExecutor`，从 input 顺边遍历，RuleRef 回调单规则求值入口 `execute(snap,ctx)`；同步、无状态、无持久化——是 DMN 类决策图，**不是 BPMN 类流程引擎**。**发布期冻结（复用 `freezeMetricDeps` 模式）**：把 RuleRef 引用规则的 ACTIVE 版本冻进 flow 快照（无 ACTIVE 拒绝发布，对齐 D6 不可变）+ metricDeps 全图并集，评估期 `EvalContextAssembler` 一次取全、零额外查询。**静态分析加维度**：图环检测 + 死节点可达性（仿静态 detector，与现有 7 类「规则集两两」维度正交）。**三条边界红线**：① 不做流程引擎（要长运行有状态编排另立项目，参考 raftkit 拆分先例，规则做被调节点）；② 不做 Function/JS 节点（zen 有 QuickJS，本项目 D60 已纯决策化，`Transform` 只允许表达式走既有 6 引擎、禁任意副作用脚本）；③ 不重写扁平模型（单条规则仍可独立存在，不强制包图）。**触发**：出现「一条规则结果喂另一条 / 多场景复用同段决策 / 决策走多步分支流水线」的真实需求（本轮用户确认此为规划中能力，故落设计稿）。**与既有「决策流不进 Rule.kind」表述的关系（命名双义澄清）**：08-evolution §2.1/§2.4 与 README D12 行早期写的「决策流由流程引擎承载、不进 kind」特指**有状态动作编排**（跨时间、要等待/人工/外部事件的 Rule→Rule 流程，接 Flowable/Camunda，仍成立）；本条 `DECISION_FLOW` 是**同步纯决策图**（一次评估内算完、无状态、高频），与前者不同物，故进 kind。判据 = 要不要「等」：要等归流程引擎，一口气算完归 `DECISION_FLOW`。本条不推翻流程引擎边界，只把「决策流」一词拆成两义。设计见 `specs/2026-07-20-decision-flow-orchestration-design.md`，实现计划见 `plans/2026-07-20-decision-flow-orchestration.md`。 |
